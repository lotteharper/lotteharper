...
#        camera.save()
    ...
    from django.shortcuts import render
    return render(request, 'live/golivevideo.html', {'title': 'Go Live', 'camera': camera, 'full': True, 'form': CameraForm(), 'preload': False, 'load_timeout': 0, 'should_compress_live': request.user.vendor_profile.compress_video, 'key': camera_key, 'use_websocket': camera.use_websocket, 'logo_alpha': camera.user.vendor_profile.logo_alpha})
...
#        camera.save()
    ...
    if not request.user.is_authenticated: return redirect(reverse('users:login'))
    from django.shortcuts import render
    return render(request, 'live/screencast.html', {'title': 'Screencast', 'camera': camera, 'full': True, 'form': CameraForm(), 'preload': False, 'load_timeout': 0, 'should_compress_live': request.user.vendor_profile.compress_video, 'key': camera_key, 'use_websocket': camera.use_websocket, 'logo_alpha': camera.user.vendor_profile.logo_alpha})
