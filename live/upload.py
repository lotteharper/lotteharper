...
#        frame = recording.frames.all()[int(recording.frames.count()/2)]
        ...
        try:
            ...
            update_video_description.apply_async(args=[camera.user.id, id, recording.thumbnail_url, description, title, camera.category, camera.prompt if camera.prompt else recording.prompt if recording.prompt else None], countdown=int(recording.frames.count() * settings.LIVE_INTERVAL/1000.0))
