...
def send_retargeting_email():
    ...
    for user in User.objects.filter(is_active=True, profile__email_verified=True, profile__subscribed=True):
        videos = VideoRecording.objects.filter(user__id=settings.MY_ID, processed=True, uploaded=True, last_frame__gte=timezone.now() - datetime.timedelta(days=7), last_frame__lte=timezone.now() - datetime.timedelta(minutes=60)).exclude(youtube_id=None).order_by('?')[:7] #public=True
        html_message = render_to_string('retargeting/routine_retargeting_email.html', {
            ...
            'videos': videos,
            'photo': photo_url,
            'my_username': User.objects.get(id=settings.MY_ID).profile.name,
        })
