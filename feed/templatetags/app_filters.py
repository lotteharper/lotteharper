...
def transpost(target):
    ...
    return translate_html(get_current_request(), post.content if len(post.content) < settings.POST_READER_LENGTH else post.content_compiled, target=get_current_request().LANGUAGE_CODE if get_current_request() and not get_current_request().GET.get('lang', None) else get_current_request().GET.get('lang') if get_current_request() and get_current_request().GET.get('lang', None) else None, src=post.author.profile.language_code if post.author.profile.language_code else None)
