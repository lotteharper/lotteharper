...
def feed_context(request):
    context_data = dict()
    try:
        context_data['lang'] = request.LANGUAGE_CODE if not ((request.user.profile.preferred_language and hasattr(request, 'user') and hasattr(request.user, 'profile') and request.user.is_authenticated) or request.GET.get('lang', None)) else request.GET.get('lang') if request.GET.get('lang', None) else request.user.profile.preferred_language if hasattr(request, 'user') and hasattr(request.user, 'profile') and request.user.is_authenticated else settings.DEFAULT_LANG
...
# hasattr(request, 'user') and request.user.is_authenticated and
        async_get_sun.delay(user.id if user else None, request.user.is_authenticated, ip.ip_address)
        sunset = ip.sunset
        sunrise = ip.sunrise
