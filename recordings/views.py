...
def recordings(request, username):
    ...
    if model == request.user and request.GET.get('all'):
        recordings = recordings_annotated.filter(user__profile__name=username, processed=True, safe=not minor_document_scanned(request.user), num_frames__gte=9).exclude(youtube_id='', file_processed=None).order_by('-last_frame')
    elif model == request.user and request.GET.get('camera'):
        recordings = recordings_annotated.filter(user__profile__name=username, processed=True, camera=request.GET.get('camera'), safe=not minor_document_scanned(request.user), num_frames__gte=9).exclude(youtube_id='', file_processed=None).order_by('-last_frame')
    else:
        recordings = recordings_annotated.filter(user__profile__name=username, processed=True, camera='private', safe=not minor_document_scanned(request.user), num_frames__gte=9).exclude(youtube_id='', file_processed=None).order_by('-last_frame')
    private_recordings = recordings_annotated.filter(user__profile__name=username, processed=True, recipient=request.user, safe=not document_scanned(request.user), num_frames__gte=9).exclude(youtube_id='', file_processed=None).order_by('-last_frame')
