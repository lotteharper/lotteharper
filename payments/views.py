...
def render_agreement(request, name, parent, mother):
    from django.conf import settings
...
def surrogacy(request, username):
    ...
    if not vendor:
        ...
        if request.user.is_authenticated: return redirect(reverse('feed:follow', kwargs={'username': username}))
    ...
    print('Agreement rendered')
    post = Post.objects.filter(public=True, private=False, published=True, feed='private').exclude(image=None).order_by('?').first()
    print('Got post')
    r = render(request, 'payments/surrogacy_checkout.html', {'title': 'Surrogacy Plans', 'stripe_pubkey': settings.STRIPE_PUBLIC_KEY, 'post': post, 'vendor': vendor, 'agreement': agreement, 'surrogacy_fee': settings.SURROGACY_FEE, 'business_type': settings.BUSINESS_TYPE, 'helcim_key': settings.HELCIM_KEY, 'form': CardPaymentForm(), 'preload': False, 'down_payment': settings.SURROGACY_DOWN_PAYMENT, 'weekly_payment': (settings.SURROGACY_FEE - settings.SURROGACY_DOWN_PAYMENT)/36})
    print('Post render')
    if request.user.is_authenticated: patch_cache_control(r, private=True)
    else: patch_cache_control(r, public=True)
    print('Serve')
