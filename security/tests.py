...
def mrz_or_nfc_verified_skey(user, session_key):
    ...
    import datetime
    from django.conf import settings
    return user and (mrz_verified_skey(user, session_key) or nfc_verified_skey(user, session_key)) or user.user_sessions.filter(bypass=True, session_key=session_key, timestamp__gte=timezone.now() - datetime.timedelta(minutes=settings.LOGIN_VALID_MINUTES), expires__gte=timezone.now(), deauth=False).count() > 0
