...
class PincodeForm(forms.Form):
    pin = forms.IntegerField(widget=forms.NumberInput(attrs={'autofocus': 'autofocus', 'inputmode': 'numeric', 'pattern': '[0-9]*', 'step': '1', 'autocomplete': 'off'}), required=True)
    def __init__(self, *args, **kwargs):
        super(PincodeForm, self).__init__(*args, **kwargs)
        self.fields['pin'].label = 'Enter your pincode'
