...
#    print(lang_code)
    ...
    trans = CachedTranslation.objects.filter(src=lang, dest=lang_code, src_hash=db_key).order_by('timestamp').first()
...
def translate_html(request, html, target=None, src=None):
    ...
    if translation is not None:
        return translation
    from .models import CachedTranslation
    trans = CachedTranslation.objects.filter(src=src, dest=target, src_hash=db_key).order_by('timestamp').first()
    if trans:
    ...
    for tag in [tag for tag in soup.find_all(True) if tag.string and tag.string.strip()]:
