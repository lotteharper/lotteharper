...
def trans(src, target, segments):
    results = [None] * len(segments)

    def thread(target, src, to_trans, count, results):
        try:
            if to_trans != None and to_trans != '':
                translator = Translator()
                translated = translator.translate(to_trans, src=src, dest=target).text
                results[count] = translated
        except:
    ...
    for count, segment in enumerate(segments):
        t = Thread(target=thread, args=(target,src,segment,count,results))
        t.start()
        threads.append(t)
...
#        except: lang = settings.DEFAULT_LANG
    ...
    result_arr = trans(src, target, content_fragments)
    result_text = ''
    pronunciation_text = ''
...
def translate_html(request, html, target=None, src=None):
    ...
    translated_results = trans(src, target, batched_payloads)
...
def translate_multiple(request, split, target=None, src=None):
    ...
    results = trans(src, target, batched_payloads)
